...
import { ManagerRegisterService } from 'src/app/services/adminServices/manager-register.service';
...
export class RegisterManagerComponent implements OnInit {
  ...
  constructor(
    ...
    private managerRegister: ManagerRegisterService
  ) {}
  ...
  registerManager() {
    ...
    if (
      ...
      this.managerRegister.pmRegister(user).subscribe(
        () => {
          this.toast.createToster('success', 'Manager Successfully Registered');
