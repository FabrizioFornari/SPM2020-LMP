...
@Component({
  selector: "app-add-personal-parking-lot",
  templateUrl: "./add-personal-parking-lot.component.html",
  styleUrls: ["./add-personal-parking-lot.component.css"],
})
export class AddPersonalParkingLotComponent implements OnInit {
  ...
  addParkingLot(form) {
    ...
    this.parkingLotService.addPersonalParkingLot(body).subscribe(
      () => {
        this.toast.createToster(
          "success",
          " Personal Parking Lot Successfully Added"
        );
      ...
      (error) => {
        ...
        if (error.status == 400) {
          ...
          this.toast.createToster(
            "error",
            "Personal Parking Lot Number Already Used"
          );
