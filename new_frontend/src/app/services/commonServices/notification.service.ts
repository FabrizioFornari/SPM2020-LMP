...
export class NotificationService {
  ...
  constructor(private http: HttpClient, private toast: NgxToastService) {}
  ...
  connect() {
    if (!this.client || this.client.connected) {
      ...
      this.client.configure({
        webSocketFactory: () => new SockJS(NOTIFICATION_CONNECT),
    ...
    this.client
      ...
      .pipe(
        map((response: any) => {
          ...
          this.toast.createToster('info', 'You have new notifications');
          return text;
        })
      )
  ...
  startNotifications() {
    if (this.client && this.client.connected) {
      this.client.publish({
        destination: NOTIFICATION_START,
